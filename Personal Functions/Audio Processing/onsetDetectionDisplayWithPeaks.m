function [] = onsetDetectionDisplayWithPeaks(y, Fs, nb_sec)
% onsetDetectionDisplayWithPeaks.m
% -------------------------------------------------------------------------
% Display the all plot's transformation between the Waveform and the 
% Half-wave rectification of the Audio signal with viewable peaks
% -------------------------------------------------------------------------
% Inputs:
...
%   nb_sec  : a positive scalar representing desired time (in s)
    ...
    [pic_valeurs, pic_positions] = findpeaks(r,'MinPeakHeight', 0.1*max(r));
